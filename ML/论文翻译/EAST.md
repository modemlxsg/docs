...
<<<<<<< HEAD
对于RBOX，几何形状由4个通道的轴对齐边界框（AABB）R和1个通道旋转角θ表示。R的公式与[9]中的公式相同，其中4个通道分别表示从像素位置到矩形的`顶部，右侧，底部，左侧`的4个距离。
=======
...
<<<<<<< HEAD
不失一般性，我们只考虑几何是四边形的情况。分数图上的四边形的正区域被设计为**缩小(shrunk)** 的原始区域，如图4（a）所示。
=======
...
<<<<<<< HEAD
如3.2所述，几何图可以是RBOX或QUAD之一。 RBOX的生成过程如图4（c-e）所示。对于那些文本区域以QUAD样式注释的数据集（例如，ICDAR 2015），我们首先生成一个覆盖整个区域且面积最小的旋转矩形。然后对于每个具有正分数的像素，我们计算它到文本框的4个边界的距离，并将它们放到RBOX地面实况的4个通道中。对于QUAD的ground truth，8通道几何图中具有正分数的每个像素的值是其从四边形的4个顶点的坐标偏移。
=======
